...
pack1_x = 50 + int(1 * 300 * 1.3) - PACK_RATIO[0] // 2
pack3_x = 50 + int(2.5 * 300 * 1.3) - PACK_RATIO[0] // 2
pack5_x = 50 + int(4 * 300 * 1.3) - PACK_RATIO[0] // 2
pack_y = 800 + int(CARD_RATIO[1] * 0.3)
...
def draw_interface():
    ...
    screen.blit(target_text, (bar_length - 370, 240))

    # Draw card slots only if cards are selected
    for i in range(5):
        if slots[i] is not None:
            card_image = card_images[0] if slots[i] == 0 else card_images[slots[i]]
...
def handle_pack_selection(pack_index):
    global total_spending, displayed_cards, start_time, slots
...
def update_slots():
    global displayed_cards, slots
    if displayed_cards and time.time() - start_time >= 2:
        for i in range(5):
            if slots[i] == 0:
